...
import { ChevronDown, ChevronRight, Loader2, Search, FileCode } from 'lucide-react';
...
export default function ThinkingCard({ toolName, toolArgs, isFinished = true, result }: ThinkingCardProps) {
    ...
    const getIcon = () => {
        ...
        return <Loader2 size={16} className="text-purple-400" />;
    };

    const getTitle = () => {
        if (toolName === 'search_web') return `Searching: "${toolArgs.query}"`;
        if (toolName === 'create_artifact') return `Generating file: ${toolArgs.filename}`;
